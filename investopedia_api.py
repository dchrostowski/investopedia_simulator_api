...
class InvestopediaSimulatorAPI(object):
    ...
    def prepare_trade(self, trade):
        ...
        submit_url = UrlHelper.set_query(self.route(
            'tradestock_submit'), submit_query_params)
        prepared_trade = PreparedTrade(
            self.session, submit_url, submit_data, **trade_info)
    ...
    def login(self):
        ...
        if resp.status_code != 200:
            raise InvestopediaAuthException("Got status code %s when fetching home %s" % (
                resp.status_code, self.route('home')))
        ...
        if len(sign_out_link) < 1:
            raise InvestopediaAuthException(
                "Could not authenticate with cookie")

        awards_url = tree.xpath(
            '//div[@class="sim-page"]//div[contains(@class,"box") and contains(@class,"info")]/div[contains(@class,"box")]/div[@class="title"]/h2/a[text()="Your Awards"]/@href')[0]
